	...
	if(userID == 0) {
		...
		if(v !== undefined) {
			...
			setTimeout(function() {
				...
				switch(v) {
					case "ads":
						msg = "Debes de crearte una cuenta para activar el adblock.";
						history.pushState({page: "settings", tab: "system"}, "", "");
						break;
					case "console-name":
						msg = "Debes de crearte una cuenta para cambiar el nombre de la consola.";
						history.pushState({page: "settings", tab: "system"}, "", "");
						break;
					case "custom-link":
						msg = "Debes de crearte una cuenta para añadir links personalizados.";
						history.pushState({page: "home", tab: "links"}, "", "");
						break;
					case "display-colors":
						msg = "Debes de crearte una cuenta para cambiar los colores de pantalla.";
						history.pushState({page: "settings", tab: "system"}, "", "");
						break;
					case "error-info":
						...
						break;
					case "notifications":
						msg = "Debes de crearte una cuenta para change notification settings.";
						history.pushState({page: "settings", tab: "notifications"}, "", "");
						break;
					case "theme":
						msg = "Debes de crearte una cuenta para cambiar el tema.";
...
function checkName(firstName, i, b) {
	...
	// Prompt the user
	name = prompt("Escribe tu nickname.\nPodras cambiarlo en otro momento.", name);
	// Check if the name is empty, and go back if so
	if(name == "") {
		alert("El nickname esta en blanco.");
			...
			success: function(response) {
				if(response.error == false) { // If there was no error
					...
					if(response.badword == true) {
						...
						alert("El nickname contiene lenguaje inapropiado.");
						checkName(name, i, b);
					}
					else if(response.length == true) {
						// If the name is too long
						alert("El nickname es demasiado largo.");
						checkName(name, i, b);
						return false;
					}
					else {
...
function newUserOK() {
	...
	if(!saving) {
		if(!granted) {
			...
			if(confirm("Para permitir usar esta cuenta otra vez, tu ip sera almacenada en el servidor.\nPara continuar, debes de aceptar.")) {
				saveNewUser();
			}
		}
		else {
					...
					if(type == "blocked") {
						...
						if(data.online) {
							if(data.online_in_app) {
								$("#online-status").addClass("online in-game").html('<img class="game-icon" src="/i/'+data.online_in_app.icon+'.png"><span><div id="online-square"></div> Online</span><div class="app-name">'+data.online_in_app.name+'</div>');
